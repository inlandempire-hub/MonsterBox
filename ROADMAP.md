...
| 9 | **0.2 Graceful-failure + manual-correction path.** ✅ **Done.** A flagged block shows a "Needs review" banner listing exactly what the importer was unsure about, with a **Fix** button that opens the editor with those fields **outlined in red**. Saving marks the creature reviewed (turns green, leaves the review list). Nothing is dropped or rejected — a 95%-right block plus a quick fix beats a failed import. | ✦⚙ | M |
| 10 | **0.3 Per-block parse-confidence indicator.** ✅ **Done.** Weighted confidence + human-readable `parse_warnings` per block (catches missing actions / unparsed abilities, not just a field count); green/amber/red dot with a reasons tooltip on each compendium row; import summary reports "N need review (red/amber)"; a **"⚠ Review" filter** + "X need review" link narrow the library to flagged blocks — the instrument for stress testing. | ✦⚙ | S |
...
4. **AI-disclosure wording (#4):** ✅ **Locked:** *"This app was created using AI-assisted
   code, however, it does not contain any AI-generated content or artwork."* Shown in the
   app's About & legal panel and the README.
